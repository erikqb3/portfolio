...
//function pathResults(path, number, url)
  ...
  if (path === 1) { 
    fetch(url).then(convertToJson).then(yourPokemon); 
  }
  else if (path === 2) {
    ...
    if (yourPokemon.length ===0) {
      ...
      fetch(url).then(convertToJson).then(otherPokemon);
    }
  }
  else if (path === 3) {
    let done = false;
    ...
    fetch(otherUrl).then(convertToJson).then(otherPokemon); 
  }
  ...
  function choosePokemon(convertedData) {
  ...
  function otherPokemon(response) {
    let jsonData = response;
